...
def presort(array: np.array, stop: str = 'undefined'):
    if array.shape[0]==0:
        return array
    ...
    while len(mins) < array.shape[0]:
        ...
        mins.append(x.argmin(fill_value=20000))
...
def sort_by_position(data: pd.DataFrame, remove_ground_vectors: bool = False) -> tuple[pd.DataFrame, float]:
    ...
    # Se puede hacer un mix: usar el aeropuerto si el primer mensaje está lejos del mismo,
    # y el primer vector si está suficientemente cerca del aeropuerto
    ...
    data['ordenInicial'] = range(data.shape[0]) # data.index # 
    ...
    stats['initial'] = distance(data[['latitude','longitude']].values).sum()
    ...
    data.iloc[:end_origin_segment] = presort(data.iloc[:end_origin_segment].values)
    ...
    windows = generate_windows(data.shape[0], window_size_slow, overlap_slow, end_cruise_segment-2, last_idx)
    ...
    final_distance = distance(data[['latitude','longitude']].values).sum()
...
def fix_timestamp(df: pd.DataFrame):
    ...
    df['fixed_timestamp'] = df.set_index(np.cumsum(distance(df[['latitude','longitude']].values))[::-1])['fixed_timestamp']\
